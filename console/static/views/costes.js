// Vista de costes: el dashboard de consumo de los agentes, tal cual era antes de que la
// consola tuviera pestañas. Lee los .jsonl de pi-cost-counter agregados por console/costs.py.
//
// Lo único que cambia respecto a la versión de una sola página es que ahora se monta y se
// desmonta: al salir de la pestaña hay que destruir las gráficas de Chart.js, que si no
// siguen vivas (y con ellas sus listeners de resize) mientras miras otra cosa.
...
const MARKUP = `
  <div class="view-head">
    ...
    <div class="controls">
      <div class="segmented" id="range">
        <button data-range="day" class="active">Último día</button>
        <button data-range="week">Última semana</button>
      </div>
      <div class="daterange">
        <input type="date" id="from" title="Desde" />
        <span class="sep">→</span>
        <input type="date" id="to" title="Hasta" />
        <button id="apply" class="ghost" title="Aplicar rango de fechas">Aplicar</button>
      </div>
      <button id="refresh" class="ghost" title="Recargar datos">↻</button>
    </div>
  </div>
  ...
  <section class="panel">
    <h2>Evolución en el tiempo <small id="evo-sub"></small></h2>
    <div class="chart-wrap"><canvas id="evolution"></canvas></div>
  </section>

  <section class="grid">
    <div class="panel">
      <h2>Reparto del coste por agente</h2>
      <div class="chart-wrap"><canvas id="share"></canvas></div>
    </div>
    <div class="panel">
  ...
  <section class="grid">
    <div class="panel">
      <h2>Coste por modelo y agente <small>· apilado por modelo</small></h2>
      <div class="chart-wrap"><canvas id="modelCost"></canvas></div>
    </div>
    <div class="panel">
      <h2>Interacciones por modelo y agente <small>· nº de llamadas</small></h2>
      <div class="chart-wrap"><canvas id="modelCalls"></canvas></div>
    </div>
  </section>
`;
...
export function mount(container) {
  ...
  container.querySelector("#range").addEventListener("click", (e) => {
  ...
  return initBounds().finally(load);
}
