...
- [`../ARCHITECTURE.md`](../ARCHITECTURE.md) — technical detail: how pi-link achieves
  cross-container discovery, how `backend` and `frontend` each ship several stack variants of
  their image, and how the backend's headless Eclipse (java variant) is wired up.
- `agents/<role>/AGENTS.md` — the context each agent itself loads on startup (its own role,
  ...
  `backend` and `frontend` have one per stack variant (`AGENTS.java.md`, `AGENTS.kotlin.md`,
  `AGENTS.python.md` / `AGENTS.angular.md`, `AGENTS.nextjs.md`) — the one
  that gets mounted depends on `BACKEND_STACK`/`FRONTEND_STACK`, see the README.
